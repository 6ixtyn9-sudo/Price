    ...
    capture_bars(
        ...
        post_process=not args.skip_postprocess,
            ...
            if not existing_df.empty:
                ...
                # Check if we need deeper history — existing data may only cover recent years
                if earliest_ts > historic_start + timedelta(days=7):
                    gap_days = max(1, (earliest_ts - historic_start).days)
                    print(f"Deep backfill: existing starts {earliest_ts.date()}, fetching {gap_days}d gap back to ~{historic_start.date()}")
