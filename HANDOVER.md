...
Red-Team Pass 2 — sector-concentration blind spot (DEFERRED, documented) → CLOSED 2026-08-06: same-scan reservation/commit discipline + `check_sector_concentration_cap` cap the sector when NVDA is open and KLAC/LRCX/AMAT are approved in one scan; see the 2026-08-06 entry.
...
Engine summary: pure-horizon exits (+ immortal guard + held=0 carve-out); dynamic ATR entry bands (knife guard 1.0 ATR/cap 200 + winner-capture 0.25 ATR/cap 100, side-aware); MAE-calibrated per-slice stops; bars_held/exit_reason + honest cost-model attribution; stale-warehouse refusal; fail-open audit visibility. (+ 2026-08-06: quote-outage warehouse-reference fallback for the knife guard with per-timeframe freshness; opening-window stop buffer on intraday attaches with paper-trade budget coherence; corporate-action quarantine at the monitor; adjustment-book integrity gate at ingest with quarantine ledger; knife cooldown (rolling stop-out journal); same-scan sector commit discipline — see the 2026-08-06 entry.)
DEFERRED (need live data or are doctrine tradeoffs, not bugs) — 3 of 5 CLOSED 2026-08-06: sector-concentration cap (→ same-scan commit discipline); explicit stale_warehouse reason (→ timeframe-aware stale helper + distinct block reasons); horizon-exact MAE (→ per-slice best-horizon sweep keys MAE to `best_h` in validate_slices.py). STILL OPEN: conviction-sizing sparsity (expected_returns table is sparse -> most slices size at neutral conviction 1.0; fix is populating expected_returns, research-side); per-lane cost model in attribution. Original note: sector-concentration cap; conviction-sizing sparsity; per-lane cost model in attribution; explicit stale_warehouse reason (showed as generic no_completed_state); horizon-exact MAE.
...
Full-system audit (antigravity pass) — NO REAL GAPS → SUPERSEDED 2026-08-06: a five-surface adversarial round found and patched 3 real vulnerabilities (quote-outage fail-open, opening-window stop floor, phantom corporate actions) plus 1 ingest-integrity gap (sub-35% special dividends); see the 2026-08-06 entry.
...
**Next steps**
- Monday verify (dated Monday 2026-08-03; outcome not recorded in this file — confirm and close): `valid_regimes` column present in `monitored_slices.csv` after the weekend discovery runs; first proof point is a "regime out of sample (...)" block in `paper_trade_log.csv` under `--regime-filter`.
- Post-fix observation window (opened 2026-07-30, matures ~2026-08-13): `signal_to_fill_bps` positive; horizon-reached exits; regime/stale blocks accumulating.
- Deferred (post-observation) — 3 of 4 CLOSED 2026-08-06 (see the 2026-08-06 entry): sector-concentration cap (`risk_group_key` groups by entry-condition, not sector → correlated semis bypassed the per-group cap) → closed by same-scan commit discipline; explicit `stale_warehouse` reason (surfaced as generic `no_completed_state`) → closed by timeframe-aware stale helper + distinct block reasons; horizon-exact MAE (calibrated to 5-bar window, conservative for horizon-3 slices) → closed by per-slice best-horizon sweep in validate_slices.py. STILL OPEN → next queue: per-lane cost model in attribution (still `default_cost_model()` at attribution.py:543, wrong for crypto/futures).
...
### 2026-08-06 — Adversarial hardening round (quote-outage rail, opening-window buffer, corporate-action quarantine), suite consolidation & vocabulary purge, adjustment-book integrity gate

**Five attack surfaces adjudicated — 3 patched, 2 verified sound**

- **Quote-outage falling-knife rail (PATCHED).** When `get_latest_price` fails (`skipped_no_price`), the adverse-fill guard previously failed OPEN — the July lesson in a new costume. Now falls back to the freshest completed warehouse bar as the adverse reference, freshness-bounded per timeframe (15m: 2h, 1h: 8h, 1d: 72h); blocks carry a distinct reason (`stale_signal_adverse_gap_warehouse_ref`) and the audit field records `adverse_guard=passed_warehouse_ref`. Reference data older than the freshness bound still fails open by design (no usable price info → do not block on nothing).
- **Same-scan sector race (VERIFIED).** With NVDA open and KLAC/LRCX/AMAT approved in one scan, reservation/commit discipline caps the sector (count 1<2 admits KLAC; 2>=2 blocks LRCX and AMAT). This closes the sector blind spot deferred at Pass 2 (2026-07-30) and in the 2026-08-01 deferred list.
- **Conviction blow-up (VERIFIED).** `compute_conviction` clips at 1.0 even at 1e9 edge magnitude; NaN edges produce a safe minimum; the notional budget cap binds. Recorded nuance: conviction saturates at edge >=3% — ranking flatness among high-edge candidates, not a leak.
- **Opening-window stop floor (PATCHED).** Intraday slices (1h/15m) whose calibrated `stop_atr_mult` sits at/near the 1.5x floor get a 1.4x session buffer, capped at 2.1x, during the 13:30–14:30 UTC window. Monte Carlo (N=20k): touch probability of a 1.5x-ATR stop in the first 8 bars ≈ 90.5% at open vs 41.5% midday — the floor was a first-hour trap, not a bug. Live pins in the book at the time: LNG 1.500, MU 1.524, UBER 1.519. Attach path and paper-trade budget path (`proposed_r`) use the same effective multiplier (coherence asserted by test). Daily timeframe never buffered; dial=1.0 disables.
- **Corporate actions (PATCHED).** Phantom-split quarantine at the monitor: |raw move|>=35% AND |adj move|>=35% with `split_factor==1.0` and `adj_factor≈1.0` on both bars quarantines the frame (reason `corporate_action_unadjusted_move`); booked splits and properly-adjusted frames pass; stale takes precedence — a 71.5h-old bar inside the 72h stale window still quarantines on the corp-action rule (pinned). Live capture additionally self-heals: a split detected in a pulled frame triggers a full-lookback re-pull.
- **Lineage adds in the same round:** knife cooldown (rolling stop-out journal — `stopout_count_within_days` + `knife_cooldown_days` RiskLimits field), pending-entry revalidation at scan start (resting-order gap re-check), capture split self-heal.

**Test consolidation + vocabulary purge (b02a037)**

- Both standalone adversarial-suite test files dissolved into existing topical suite files under behavior-named selectors: 25 hardening proofs + 7 relocated tests appended across `test_stops.py`, `test_stop_manager.py`, `test_risk_stops_gate.py`, `test_state_unavailable.py`, `test_sizing.py`; 6 duplicate/local-reimplementation tests retired with 1:1-or-better replacement coverage recorded. Arithmetic verified on both trees: 594 − 6 = 588.
- Campaign vocabulary from the adversarial round — the "R-team"-style section banners and the "G-prefixed" surface labels — purged to zero from every Python file (names, comments, file names) so future adversarial rounds find no bait strings; the exact banned pattern lives in the operator's sweep tooling, not in this repo. Prior-session doc references and commit messages are intentionally left archival.

**Adjustment-book integrity gate (40e53d3) — special-dividend wound closed at ingest**

- The 35% monitor quarantine cannot see ex-date specials below 35%. Pre-fix probe: a 3-bar frame with a booked $12 dividend on $100 and a flat `adj_factor` persisted silently and produced a −12.5% adjusted-space phantom crash.
- `price.warehouse.adjustment_integrity_violations(df)`: on a DAILY frame, booked corporate actions (`split_factor` != 1 by >0.5%, or `dividend_cash` >= 1% of prior close) must move `adj_factor` by the expected ex-date multiplier (tolerance max(0.2%, 25% of expected step)); `adj_factor` steps >= 1% with no booked action are flagged phantom. Fail-open on unauditable frames: missing bookkeeping columns, <2 bars, NaN/zero/negative factors on the boundary.
- Gate at `save_to_warehouse` per (symbol, timeframe) group: a violating 1d group is NOT persisted; a JSONL record is appended to `localdata/warehouse/_adjustment_quarantine.jsonl`; all other groups save normally. Intraday frames are ungated (propagate fixes their factors once dailies land).
- 17 tests appended to the existing `tests/test_warehouse.py` (no new files): poisoned split/dividend flagged, booked-and-adjusted passes, sub-materiality tolerated, phantom step flagged, flat dummy (futures/crypto pattern) never trips, intraday ungated, poison isolation in multi-symbol saves, ledger contents verified.
- Suite: 605 passed on the canonical repo (604 + 1 env-conditional skip in the agent sandbox).

**Process — two-tree convergence restored**

- Canonical record: `github.com/6ixtyn9-sudo/Price` @ 40e53d3. The agent sandbox is now a shallow clone of it. Incident: the previous sandbox `.git` (668 MB) was evicted by a workspace snapshot size cap mid-session; no work was lost because everything uncommitted had already been ported, and the sandbox was rebuilt from remote after a per-file loss audit.
- This gate ported Mac-side as a byte-exact `git apply` patch; post-push verification: zero-line diff between sandbox-generated content and the pushed tree. Doctrine update: confirm on origin/main AND diff-scope the port before accepting "shipped".

**Residuals (explicit)**

- A sub-35% unadjusted gap arriving through the Alpaca-daily fallback with NOTHING booked has no event to assert against at ingest — mitigated by intraday propagation from yfinance dailies + the 35% monitor quarantine; no live exploit in the current book.
- `_stale_warehouse_reason` (lineage helper) lacks an explicit `df.empty` guard; protected by upstream empty checks — logged as a minor edge.
- First live proving signal for the gate: watch for `_adjustment_quarantine.jsonl` entries in nightly capture.
- Next queue: per-lane cost model in attribution (`attribution.py:543` still `default_cost_model()`) — last open item from the 2026-08-01 deferred list. Conviction-sizing sparsity (research-side `expected_returns` population) remains open. Observation window matures ~2026-08-13.

**Addendum (later 2026-08-06) — "sleep items" closed before observation**

- **Adjustment provenance gate (frame-vs-history): the middle band is closed.** Pre-fix probe: a full-dummy Alpaca-fallback frame silently overwrote adjusted yfinance history on shared dates (3 of 4 rows rewritten to factor 1.0 — keep-last dedup did the damage below every detector threshold). Two contradiction checks now run before any 1d merge: (1) a full-dummy frame landing over dates whose stored rows carry real adjustment factors is quarantined; (2) two genuinely-adjusted sources disagreeing *discontinuously* at shared dates (adjusted-close ratio steps >0.5% at a shared boundary) are quarantined rather than silently picking a lineage. Vendor-refresh safety: checks compare ratio *discontinuities*, never levels — global restatements pass (pinned). Healing direction open: a genuinely-adjusted frame over dummy history repairs it (split self-heal pattern). The irreducible band is now enumerable, not silent: a full-dummy equity frame with NO contradicting history persists (fail-open on missing data) but logs `kind=unverified_provenance` to the same ledger.
- **Per-lane cost model in attribution: CLOSED.** Cross-asset round trips were net-priced with the equity cost model, flattering crypto/futures `net_of_cost` figures. `cost_model.LANE_COST_BPS`: equity 13bps RT bit-IDENTICAL to the long-standing default (zero behavior change for the equity book); crypto 40bps RT; futures 20bps RT — both conservative first-pass placeholders. Calibration TODO at the ~08-13 review: refit lane constants from realized per-lane `signal_to_fill_bps`.
- **Placeholder excision (2026-08-06, evening follow-up): no guessed constants anywhere in the system.** The crypto/futures lane values shipped in `1cc3ae1` were guesses for lanes with zero order flow — deleted outright rather than left to mature. `cost_model` is now equity-only: `cost_model_for_lane`/`cost_model_for_symbol` raise `UnsupportedLaneError` on any non-equity lane (this also closes a silent `.get(lane, equity)` fallback that would have priced a typo'd lane at equity costs). Attribution degrades inactive-lane slices to `net_of_cost_return = None` with an explicit `net_of_cost_note` plus a report-level note — gross figures stand, a net is never fabricated. Equity 13bps RT is unchanged and bit-identical; its calibration loop (realized signal-to-fill gap, maturing at the ~08-13 review) is untouched. If a crypto/futures lane ever activates, its constants are calibrated from realized fills BEFORE the first order, introduced in the same commit that activates it. Same pass: `_stale_warehouse_reason` gained a top-level None/empty-frame guard (fail-open; previously guarded only by caller convention), closing the last latent-crash debt item. Suites: Mac 616 passed; sandbox 615 passed + 1 env-skip.
- **Cross-timeframe position claiming fixed (2026-08-07): the slowest live thesis steers a shared symbol.** Diagnosed from the KLAC journal: a broker account holds ONE net position per symbol, and exit management claimed that position with the MOST RECENT entry's context — so a fresh 1h/15m entry moved still-open daily swings onto the intraday clock (validated multi-day edges realized as hours-long trades; the daily slices' scorecards were written by the fast rulebook — 1d lots exit-labeled "held 3 bars >= 3 (1h)"). Fix: `position_manager.claiming_entry_rows` — among entries with quantity still open (filled entries minus FIFO-allocated filled exits), the SLOWEST timeframe claims the pile (ties resolve to the most recent; fully-offset symbols keep legacy recency as pure fallback). The same rule drives `monitor._load_open_position_slice_labels`, so the EVALUATED stable filter belongs to the same claimant — fixing only one side would have left a daily clock under an hourly filter. Faster lots sharing a symbol remain floored by the symbol-level broker stop and the eod profit lock. Scorecard honesty: attribution tags every round trip with `exit_context_timeframe` (parsed from the exit reason's own tag) and its report note counts round trips exited under a different timeframe's rulebook — mixed-policy results stay visible instead of being absorbed. True per-lot management (broker-level virtual sub-positions) is the documented future architecture if the shared book scales. 4 new tests (3 claiming + 1 attribution flag).
- **Near-floor live pin converted to a rotation-proof invariant (2026-08-07).** The 2026-08-06 pin asserted exact symbol membership ({LNG, MU, UBER} near the intraday stop floor) against the monitored book; the nightly research-refresh legitimately rotated that book (LNG repriced to 4.0, MU to 1.646, UBER still floored) and the pin went red at pristine HEAD with zero code changes. Membership pins against a rotating managed artifact were the wrong shape. The test now asserts the durable invariant — every near-floor intraday row in the CURRENT book is strictly lifted by `effective_stop_atr_mult` inside the cash-open window — and skips loudly when the book has no such row (buffer unexercised that day, not broken). Mac suite after pull will show 1 failed (the old pin) BEFORE this patch applies — expected, it is the rotation, and this patch contains its replacement. Suites: Mac 620 passed; sandbox 619 passed + 1 env-skip.
- **Entry-revalidation crash on the resurrected scanner — six latent defects in one never-executed path fixed (2026-08-07, Actions run 31193346631):** the first equities run after the main()-tail resurrection was a loud exit-1, which is the correct failure posture: `_revalidate_pending_entries` executed for the FIRST time ever (it was introduced by c83777f — the same commit that ruptured main() — so the rupture had shielded it from ever running) and died on `KeyError('client_order_id')`: `get_open_orders()` exposed only order_id/symbol/qty/side/type/status/timestamps. Adversarial review of the block found five more latent defects stacked behind the crash: the loop read `row.get("id")` while the frame key is `order_id` (would have issued silent "None" cancels); `limit_price` was absent from the frame (every row would have been NaN-skipped — see also the next); the entry filter matched `client_order_id.str.contains("ext_")`, a substring no code path in the repo has ever produced (the function was a guaranteed permanent no-op even after the crash fix); `from price.features import compute_atr_14` (that name lives in price.sizing — ImportError on the first stale candidate); and `from price.risk_limits import resolve_adverse_threshold_bps` (that name lives in price.trading and was already imported at module top — ImportError on the first stale candidate). Shipped fixes: `get_open_orders` now exposes `client_order_id`/`limit_price`/`stop_price` (additive only; protective stops are submitted with NO client id, so the `price-{lane}-` prefix + `type == "limit"` filter selects exactly this lane's resting entries and can never cancel a position's only protection, nor another lane's orders); a schema-drift guard now skips ONLY this guard with a WARNING line instead of killing the whole pass; `order_id` key and both correct imports used throughout. RED→GREEN with 3 new tests: the schema-drift test reproduces the exact production KeyError locally, the real-frame test proves only this lane's stale entry is cancelled (crypto-lane entry + ASML protective stop untouched), the enum-suite test pins the new get_open_orders schema. Suite 643 (Mac). Owned blindspot: the port-1 e2e stubbed `get_open_orders` with an EMPTY frame, exercising only the early-return path and masking the entire block; the repo now drives the real production frame shape through it. Deploy-ordering lesson ledgered for the 2026-08-13 review: a structural resurrection fix should be followed by a forced live pass within the hour, because the next scheduled run may be the first true execution of long-dormant code — exactly what happened here (run manually dispatched early, crash surfaced in ~13 minutes instead of possibly days later on a rare non-empty-orders coincidence).
- **Equity scanner resurrection + daily-frame unfreeze + per-lane warehouse caches (2026-08-07, from the operator noticing the equity dashboard went quiet).** Three production defects, each proven with executable evidence. (1) RUPTURED ENTRYPOINT: `scripts/paper_trade.py`'s `main()` ended at the cost-model print — a column-0 `def _revalidate_pending_entries` introduced 2026-08-06 ~17:27Z in c83777f's hardening pack split the function; everything below (the scan, entries, exits, stop management, and the just-shipped heartbeat) compiled as orphaned, never-called code, so `sys.exit(main())` resolved to `sys.exit(None)` — a silent exit 0. Byte-disassembly proof: `main` ends `RETURN_CONST None`; a sandbox reproduction matches production's five-lines-then-silence profile exactly; a line tracer stops at the cost-model print. Every equity run 2026-08-06 ~17:27Z → repair was a ~1.3-second no-op — the committed ledgers show no decision rows, no heartbeats, no orders in that window (re-confirmed live on the 2026-08-07 14:17Z run). The parked crypto/futures lanes kept heartbeating only because their park branch sits above the rupture. The suite stayed green because it calls helpers directly; nothing exercised `main()`'s tail. Fix: structural surgery — `_revalidate_pending_entries` is now a proper module-level sibling while `_one_pass` and the loop tail are nested in `main()` as intended; a post-fix run with fake credentials fails LOUDLY (rc=1, full traceback) instead of dying silently. (2) VENDOR FREEZE: Yahoo began zero-filling the `Stock Splits` actions column server-side around 2026-08-03; verified across yfinance clients 1.4.1 / 1.5.1 / 1.5.2 — every client returns `[0.0, ...]`, so the pin is not at fault and NO pin revert can fix it (the operator-approved revert was deliberately NOT shipped; yfinance stays 1.5.1 — ship no change that cannot change behaviour). A zero factor reads as a phantom ratio-zero split: the incremental-pull split detector misfired on all 139 symbols and the full re-pulls then violated the warehouse adjustment-book gate on every boundary, so every daily frame was refused since 2026-08-03 (139/139 red-stamped on the 2026-08-07 captures). Fix: ingest-boundary normalisation in `_build_yfinance_canonical` — split ratios must be strictly positive and dividends non-negative; malformed vendor values become the "no event" value, matching the fail-open doctrine both downstream gates already apply; genuine ratios and dividends survive. (3) CACHE POLLUTION: all three live-capture lanes shared one bare warehouse-cache restore prefix — the 2026-08-07 equity run provably restored the FUTURES lane's warehouse (`research-warehouse-v4-Linux-live-capture-futures-*`) — and equities never SAVED a warehouse cache at all, guaranteeing permanent cross-lane consumption. Fix: lane-scoped keys everywhere (equities gains the `live-capture-equities` tag plus its own hourly save; crypto/futures restore keys scoped to their own tags). The first equity restore post-deploy is cold by design — its capture rebuilds the warehouse in-run; crypto/futures cold paths seed from the release. Impact framing for the 08-13 review: equity executions recorded nothing from 2026-08-06 ~17:27Z until this deploy lands; daily evidence bars are frozen at 2026-08-03 until the unfreeze is confirmed — watch the next equity capture for ~139 `Warehouse saved ... | 1d` lines, zero adjustment-book violations, and `scan_complete` heartbeats in `localdata/paper_trade_log.csv`. During the dead window the open book was protected only by broker-side stops. No slice verdict may quote the frozen/dead window. Tests: +2 — an end-to-end `main()` scan-to-heartbeat guard in `test_paper_trade_stop_audit.py`, and zero-split ingest normalisation in `test_sources.py`; each verified RED on the pre-fix tree and GREEN after. Suites: sandbox 639 passed + 1 env-skip; Mac 638 baseline → 640 expected.
- **Lane confinement, lane parking, research-freshness gates + scan heartbeat (2026-08-07, from the operator question "why is there no action on the crypto/futures books?").** Findings, all from committed ledgers / workflow definitions / run telemetry: (1) The futures book was empty by an HONEST gate (131 candidates, zero strict passes; the single marginal neutral candidate is tagged `parent_underperformed`) — yet the futures lane had traded anyway: 6 real equity ETF entries (SPY 1h, XLF 1d, ~$975 notional, broker-accepted) between 2026-07-27 and 2026-08-05, submitted from stale ops-cache-resurrected candidates the current gate would never emit, journaled only to `trade_journal_futures.csv` — a ledger the equity attribution never reads (it is hardwired to `trade_journal.csv`). On 2026-08-05 Alpaca rejected its retries for duplicate `client_order_id`. XLF is flat now; those round trips are contamination, not equity evidence, and stay excluded from the 08-13 edge validation. (2) The crypto lane fired 72 short entries on 2026-07-26/27, all broker-rejected (spot cannot short: "insufficient balance"); the long-only filter then left a 1-slice book (AAVE/USD 1d) whose condition never fired and whose evaluation rows stop after 2026-08-01 00:36 — the latest capture ran green in 75 seconds with a 1-second paper-trade step and zero new ledger rows (a zombie: green was indistinguishable from working). Its research summary is frozen at 2026-07-15; the daily refresh swallows failures via `|| echo "regime-only rebuild failed"`. (3) Shared-account bleed: the crypto stop-state file babysits ASML (an equity); both lane journals hold the other lane's order rows (stop reconciliation sees the broker account, not the lane). (4) The equity lane is provably exempt from the ghost vector: its ops cache restores ONLY ops files (no research/), and its book flows from git-committed discovery artifacts. Shipped: `src/price/lane_guard.py` — fail-closed lane/symbol rules enforced in `submit_entry` BEFORE any broker client is constructed (equity lane: plain symbols only; crypto lane: `/USD` spot pairs only; futures lane: submits nothing, no execution path exists; unknown lanes get no broker access), refusal journaled in the lane's own ledger as evidence. `scripts/research_freshness.py` wired into both lane book-sync scripts: books build only from research stamped within 72h; stale/missing/unparseable stamps wipe the book to header-only with `::notice::` (a ghost book can no longer persist quietly; expect the stale crypto AAVE row to be wiped on the next sync — that is the point). `scripts/paper_trade.py` heartbeat: exactly one `scan_summary` audit row per run — `lane_parked` / `scan_complete` (book_size, signals_total, all counts, runtime_s) / `scan_failed` — and emission can never crash a scan; quiet green and working green are now distinguishable in the lane ledgers themselves. `localdata/PARK_LANE_CRYPTO.flag` + `PARK_LANE_FUTURES.flag` committed (operator decision: prove the equity book first; crypto needs a short-capable venue decision AFTER the 08-13 review — its discovered edges are ~93% short-side; futures has no broker and zero gate candidates). Parking skips scan/broker work only; hourly ingest keeps running so neither warehouse cold-starts on unpark (delete the flag to unpark). Test-infra fix in the same pass: `test_substrate_isolation.py`'s bare `alpaca` root stubs were order-dependent collection landmines (reproduced: collecting it before any `price.trading` importer dies with 'alpaca' is not a package); stubs now load the real package root and keep only leaf shadows. 18 new tests (4 lane-shape in substrate, 3 broker-chokepoint in trading, 7 freshness/sync, 4 heartbeat/park). Suites: Mac 638 passed; sandbox 637 passed + 1 env-skip.
- **Doctrine additions (from the candid review, now written):** every new guard ships with its adversarial counter-test; data provenance is asserted, never trusted.
- Remaining open: conviction-sizing sparsity (research-side); adjusted-price point-in-time ratio semantics (documented approximation — trading-grade, not audited accounting).
- **DST correction: the opening-window buffer now follows the New York wall clock.** The 08-06 implementation pinned the window at fixed 13:30–14:30 UTC. That is correct under EDT — and from 2026-11-01 (EST) it would have widened 08:30–09:30 ET and left the actual cash open UNPROTECTED for five months (the old comment's "safe side" claim was wrong in the dangerous direction). Old-vs-new proof on winter pins: at 09:31 EST the old code returned 1.5 (floor slice unprotected) and at 08:31 EST it returned 2.1 (needlessly widened premarket); the `zoneinfo` America/New_York window returns 2.1 and 1.5 respectively. Seam contract: naive datetimes in tests are read as UTC so the host zone cannot leak into window math. 1 DST regression test appended to `test_stops.py`. Suite after this commit: 615 passed on canonical repo (614 + 1 env-skip in the agent sandbox).
- 9 new tests appended to existing suite files (6 provenance in `test_warehouse.py`, 3 lane in `test_attribution.py`) — noted above; the DST correction adds 1 more.
- **Vendor basis flip: three independent defects under one daily-frame freeze, fixed after probes — never before (2026-08-07 evening, from run 31201571453's 123 adjustment-book refusals).** Hours after the zero-guard unfreeze, the 17:17Z capture logged 123 `Adjustment-book violation` refusals and only event-free/cold daily books saved — corp-action daily books frozen again. Per doctrine nothing was patched until raw probes (sandbox, yfinance 1.5.1, no engine code, repeat-fetched for CDN stability) settled the vendor facts: Yahoo now serves OHLC and Volume SPLIT-ADJUSTED (continuous across ex-dates) with events still booked — AMZN 20:1 Close 122.35→124.79 flat with 20.0 booked; AVGO 10:1 170.067→171.42 with 10.0 booked. Adjusted Close keeps the standard convention (pre-ex prices adjusted down; the factor steps UP across ex-dates — ABBV 0.9848→0.9929 over the 2026-04-15 ex-date). Dividend cash is served split-adjusted (AVGO 2019 $2.65 served as 0.265; 2024 $5.25 as 0.525); volume is served forward-adjusted (AMZN 97.6M pre-split ≈ 4.88M as-traded × 20). NO history()/download()/back_adjust knob serves as-traded raw anymore, and repeat fetches are byte-identical — a clean vendor change, not a rollout flake. The 123 refusals decompose: (1) SPLIT-CLASS (18) — continuous served prices + booked split = flat adj_factor next to a booked event; the integrity gate correctly refused (vendor defect, gate did its job). (2) DIVIDEND-CLASS (37) — the gate's dividend term was INVERTED since it shipped (40e53d3): it expected the factor to step DOWN ×(1−dv/pc) while every real adjusted series steps UP ÷(1−dv/pc); the live refusals are exact reciprocal pairs (observed 1.0121 vs expected 0.9880; 1/0.9880 = 1.01212). Our latent defect: the gate shipped 2026-08-06 20:38Z, zero-filled splits froze daily frames until the zero-guard, so no genuine material dividend was ever audited before 2026-08-07 — and the shipped tests encoded the inverted convention, which is why CI stayed green. (3) DUMMY-CLASS (65) — post-flip, factor == 1.0 with adj == raw and no booked events is the STEADY STATE for every bar after a stock's latest corporate action, so any honest short incremental window is frame-locally a "full dummy" and the provenance gate's blanket dummy-over-adjusted-history refusal froze healthy books; persisting can only rewrite dates the frame actually covers, so the blanket was over-refusal. Shipped fixes: (1) `_build_yfinance_canonical` reconstructs the as-traded basis per booked split — pre-ex raw OHLC × ratio, pre-ex dividend cash × ratio, pre-ex volume ÷ ratio — with per-event detection from the frame itself (served adj/close factor steps ~1 over the ex-date on the adjusted basis, ~the ratio on as-traded), so honest as-traded frames pass through untouched and close_adj (already continuous and scale-correct) is never touched; (2) gate expected-step term corrected to sf / (1 − dv/pc) with a junk guard (cash ≥ close = unauditable boundary, not a breach); (3) dummy refusal narrows to OVERLAPPED stored rows carrying real adjustment bookkeeping (factor ≠ 1 or a booked event on a covered date) — a dummy over factor-1.0, event-free dates persists and still writes its `unverified_provenance` ledger note. Fail-closed posture unchanged wherever ambiguity is real: anything the reconstruction cannot make self-consistent is still refused. Executable proof: RED — 8 focused tests fail on the pre-fix tree, including fixtures with verbatim live numbers (AMZN 20:1 frame; ABBV's verbatim production refusal values; AAPL dummy-window-over-adjusted-history); a live end-to-end on the REAL engine path pre-patch reproduces AMZN's refusal byte-identically to the production log line and shows AVGO refused at a dividend (reciprocal signature) before its split is even reached. GREEN — suite 648 passed + 1 env-skip in the sandbox (baseline 642 + 1; +6 new tests: 3 warehouse, 3 sources; 4 inverted-convention fixtures converted; 1 overlap pin), and the post-patch live end-to-end lands all four scenarios: AMZN's book holds as-traded 2447.00 on 2022-06-03 with the 20.0 event booked; AVGO holds 1700.67 pre-split and the $5.25 as-traded dividend (0.525 × 10); ABBV persists with its dividends booked; an AAPL book seeded to 2026-07-15 grows 1892→1910 rows off the honest 26-row incremental window. Deliberately NOT shipped (bloat doctrine): warn→ledger machinery for mass refusals — the per-symbol quarantine JSONL is already the enumerable honesty trail and refusal classes stay visible to the weekly human greps. 15m/1h need no parallel patch: only 1d frames are gated and intraday factors propagate from the healed daily books; the hourly yfinance path shares the same canonical builder, so reconstruction covers split-carrying hourly windows identically. Residual suspicion, honestly flagged: 3 `unbooked_adj_factor_step` refusals in run 31201571453 are not explained by the three classes above — the post-deploy log decides whether they recur, and if they do they get their own probe before any further patch. Verification on the next equity capture: ~139 `Warehouse saved ... | 1d` lines, violations → ~0, and `unverified_provenance` ledger notes (one per honest short-window save — expected noise, not refusals). Fixture-blindness lesson, third entry this week: the AAPL no-events fixture was maximally blind twice, and now the shipped gate tests encoded an inverted convention no real dividend had ever exercised — fixtures must carry verbatim production/vendor values, which all six new tests do. Suites: sandbox 648 passed + 1 env-skip; Mac 643 baseline → 649 expected.
- **Zero-execution sweep + the 08-13 review agenda (2026-08-07).** Full audit of the 25 audit-action literals against every committed ledger: 11 observed live in equity production; 14 never executed anywhere — 4 are dry-run twins of live actions (innocent by construction), 6 are rare-but-tested paths, and 4 go on the shortlist for the 2026-08-13 maturity review: `force_close_failed` (stop_manager.py:176), `stop_deferred_close_pending` (stop_manager.py:394), `would_ratchet_stop` (stop_manager.py:521), `would_cancel_stale_entry_order` (paper_trade.py:823). Pre-registered finding, verdict owed BEFORE any real-money gate decision: `stop_ratchet_failed` appears 30x in equity production ledgers while `stop_ratcheted` has NEVER appeared — innocent (no position has yet crossed breakeven_trigger_r = 1.0) or the ratchet path is silently broken; the autopsy decides which from the stop-order state trail, and the four shortlisted actions get the same alive-or-dead verdict. Also on the agenda: the KLAC 750e15ef metadata-gap specimen (round trips sharing one exit_order_id with null bars_held / empty exit_reason); research-side workflows still sharing the legacy warehouse cache namespace; "green != booked" (run 31201571453 was green while refusing 123 daily frames — step success must never again be the trusted signal); and the scorecard-file rename offer. Full working text lives in the operator's workspace doc zero_execution_sweep_2026-08-07.md; this bullet is its canonical pointer.
- **Adjustment-gate threshold crack closed (2026-08-07, late — from the seven remaining refusals on run 31210786362).** The basis-flip triple repair went live and was verified in-run (committed ledger: `no_state_data` 102 -> 9 on run 31208649825; heartbeat #4). The next scheduled run then saved 132 of 139 daily books with violations down from 123 to exactly 7 — all one signature: `unbooked_adj_factor_step` ~1.0100-1.0101 on a single historical ex-date per book (ABBV, CVX, DUK, GILD, INTC, MRK, SO). Raw probe first (yfinance 1.5.1, both the dividends endpoint AND the history actions cells): every one of those dates carries a properly BOOKED dividend whose yield lands at 0.991-0.9997% — inside a ~10bps crack between the gate's 1% event-materiality line and its 1% unbooked-step flag, so the gate was flagging its own booked event as "no booked corporate action". OUR defect, the fourth this week in one seam — not a third vendor malformation; the unexplained trio from run 31201571453 was this same crack (each of the 7 saved + crack-refused counts reconciles exactly: 132 + 7 = 139). Fix in `adjustment_integrity_violations`: any booked event that EXPLAINS its factor step (`sf / (1 - dv/pc)` within the tracking tolerance) is clean at any materiality; materiality now only selects the violation kind — a booked event that cannot explain the step still flags (mismatch when material, unbooked-step otherwise), and unbooked >=1% steps flag exactly as before. RED->GREEN: the live wide ABBV frame reproduces the verbatim production refusal against the pre-fix code; +2 tests (the verbatim ABBV 0.991% crack fixture now passes; an immaterial booked dividend beside an unexplained 5% step still flags). Live GREEN re-run across all seven cracked books: 0 violations each (MRK additionally exercised the split-basis reconstruction — a 2000s-era split row — cleanly). The four `no_warehouse_data` ledger symbols from run 31208649825 (ABBV/DUK/GILD/SO) are four of these seven — their partitions were never rebuilt because every wide pull since the cold-cache rebuild cracked on those dates. Post-deploy verification: the run after next shows ~139 `Warehouse saved | 1d`, zero violations, and those four symbols' skips leave the equity ledger. Suites: sandbox 650 passed + 1 env-skip (baseline 648); Mac 649 -> 651 expected.
- **Crypto live lane: 7-run overnight outage — one dormant cold-boot landmine, zero blast radius (2026-08-08, runs 31224324136 through 31239781602).** Crypto's first-ever failure streak: 7 consecutive red runs, all exit-1 at "Ingest crypto universe" in under 1 second with ZERO step output. Full operator-log autopsy: the lane's warehouse caches were all gone (restore: "Cache not found" for both the run key and the lane prefix, ~2h after the last green run SAVED its cache; eviction under the repo's churning 10GB quota is the standing suspect — the newest-2-per-family prune keeps its own lane heads, and the tiny ops-state caches survived the same window; Caches-page evidence pending), the seed fallback then silently no-oped because release `warehouse-v1` has never existed / no longer exists (repo has zero releases), so `localdata/warehouse/` was never created — and the ingest step's bare `PARQUET_COUNT=$(find localdata/warehouse ... | wc -l)` fails on a missing directory under the runner's `bash -e -o pipefail`, with its stderr swallowed by the line's own `2>/dev/null`. Silent, instant, fail-closed — by accident, not design. The landmine predates this week but was unreachable: pre-lane-scoping, any miss fell back to cross-lane research caches that materialized the directory; the (correct) per-lane cache fix removed that accident and exposed it. RED->GREEN proof with the runner's exact shell flags: bare variant exits 1 with no output; prepending `mkdir -p localdata/warehouse` takes the designed cold branch (1825d 1d / 365d 15m Alpaca backfill) and proceeds. Futures survived the same cache eviction (its ingest has no bare-find count); equities ingest shares neither pattern — Monday risk is a slow cold rebuild at worst, not this crash. Landmine grep: the unguarded PARQUET_COUNT pattern is unique to `live_capture_crypto.yml`. Collateral defects ledgered, none patched blind: (1) **dead seed** — every workflow's "Seed warehouse from release" is a silent no-op; decision owed: build the `warehouse-v1` release asset or delete the seed steps (bug hid because cache chains kept every lane warm until tonight); (2) **crypto deep research dormant since ~Jul 25** (5 discovery runs ever; the refresh workflow's `allow_discovery` gate defaults closed and the nightly dispatcher never opens it) — the sync staleness refusal (72h) has been writing header-only books for ~12 days while every run stayed green: the crypto book has silently traded nothing for a fortnight. "Green != live book" joins "green != booked" on the 2026-08-13 agenda. Blast radius tonight: exactly zero — paper trade, forward capture, guards, cache saves, and commits all skipped; the hourly dispatcher was left running (failures cost ~1 minute and will confirm the fix the hour it ships). Fix shipped: mkdir guard in the ingest step; suite untouched (workflow-yaml only), verification is field-check on the next crypto run: "cold warehouse (0 files) — full backfill", capture proceeds, lane cache re-saved, subsequent runs warm.
- Residual from the same night, parked for the cache-namespace port: WHY the crypto lane caches vanished inside ~2h (eviction vs prune) is unanswered until the Actions -> Caches listing is captured; the Saturday research-side 15m collapse (71,649 -> 13,359 discovered rows) and tonight's outage share the same cache-layer root system and are being fixed as one cache-namespace port.
- **Cache-namespace unified repair (2026-08-08): weekly wipe scoped to its own lineage, research restores scoped to research families, cross-lane restores now fail closed.** Both weekend incidents trace to ONE job: `research_refresh_equities.yml`'s `cache-maintenance`, which deleted EVERY cache in the `research-warehouse-v4-` namespace except its own current save, every Friday night (~22:07Z, run 31222202649). Friday 2026-08-07 sequence, now fully reconciled against the Actions -> Caches listing: the wipe erased the crypto lane's warehouse cache (saved 21:36Z) -> crypto run 31224324136 cache-missed at 23:27Z, cold-booted into the bare-find landmine from the bullet above, 7-run outage; the wipe erased futures' real lane cache -> the futures lane has chained ~200-byte EMPTY caches hourly ever since (green runs on a hollow warehouse: the week's second "green != live" specimen, repopulation rides its normal research path); the wipe erased both equities-lane caches (Monday will cold-rebuild once; equities ingest carries no find-landmine); and Saturday's 15m discovery (run 31222703862, dispatched 22:07:59Z) restored the newest bare-prefix survivor — the EMPTY futures-lane cache saved at 22:47Z (shard restore steps ran 2-3s; a 150MB deep restore takes materially longer) — and starved: 71,649 -> 13,359 discovered rows, 178 of 219 symbols lost, merge committed 01:28Z. Corrections recorded per doctrine: the quota-eviction theory reported overnight is dead (repo total ~330MB, the deleter was our own cleanup job, and prunes elsewhere don't exist — only the crypto live lane ever prunes); the shard-cache fingerprint was the empty futures lane, not the equities lane as first called. Shipped in this port: (1) equities-refresh cache key gains the `-equities-refresh-` infix and its cleanup jq deletes ONLY its own lineage (infix + the legacy bare run-id keys it used before today) — cross-lane hygiene stays with the crypto lane's keep-newest-2-per-family pruner, the only mechanism that never harmed a neighbor; (2) all 13 research-warehouse restore sites across the 9 research workflows now carry one ordered research-family restore list (research families first, the legacy bare prefix last, live-lane prefixes never listed); (3) every research warehouse restore step is `id: warehouse` and is followed by a "Refuse cross-lane warehouse cache" step that fails the run with ::error:: if `cache-matched-key` is a live-lane cache — silent starvation of a weekly discovery is no longer a possible posture, in any lane, under any future cache accident. Proof (sandbox + replayed on canonical): the guard probe under the runner's exact shell flags exits 1 with ::error:: on a live-lane key and passes research-family keys; the patcher asserted all 13 sites (9 workflows) before writing; all 15 workflow yamls parse post-edit; census greps: live-capture prefixes in research yamls = 0, guard occurrences = 13/9, equities-refresh key + scoped cleanup present 1/1. Field verification (no suite needed; workflow yaml only): the next research run's "Cache restored from key:" names a research-family key; next weekend's 15m discovery manifest returns to 71k-class; any future lane-cache accident fails loudly at the guard instead of starving quietly. Parked for the 2026-08-13 review: (a) 15m depth recovery — the surviving 150MB legacy cache (research-warehouse-v4-Linux-31222202649) stays reachable via the bare list-tail until its unused-TTL (~08-14); recommended default is letting the scheduled Friday refresh rebuild depth deterministically (~52-min cold precedent) rather than hero-migrating cache content — decision owed; (b) the dead `warehouse-v1` release seed — rebuild the asset or delete the seed steps everywhere (proven dead weight during the crypto outage); (c) futures lane's 200-byte empties until repopulated — harmless (research-only lane) but must not be taken for health; (d) "green != live book" joins "green != booked" as the standing posture lesson. Suites untouched (workflow yaml + this doc only).
